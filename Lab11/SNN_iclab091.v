...
reg [20:0]  div_in0		            ;
reg [15:0]  div_out		            ;
// reg [19:0]  add_in1	    	    [0:8];
// reg [19:0]  add_in2	    	    [0:8];
// reg [19:0]  add_9_in	    	    [0:8];
// reg [31:0]  add_9_out;
...
//                CLK Gating                    //
//==============================================//
//SLEEP_CTRL_in == 0, clk on
//1. img input
wire SLEEP_CTRL_in_img = (cg_en)? ((current_state == CON_TO_FULLY || current_state == DISTANCE || current_state == OUTPUT) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_img;
GATED_OR GATED_in_img(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_img), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_img));

//2. kernel input
wire SLEEP_CTRL_in_ker = (cg_en)? ((current_state == LOAD_IMG_ONLY || current_state == CON_TO_FULLY || current_state == DISTANCE || current_state == OUTPUT) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_ker;
GATED_OR GATED_in_ker(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_ker), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_ker));

//3. weight input
wire SLEEP_CTRL_in_wei = (cg_en)? ((current_state == LOAD_IMG_ONLY || current_state == CON_TO_FULLY || current_state == DISTANCE || current_state == OUTPUT) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_wei;
GATED_OR GATED_in_wei(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_wei), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_wei));

//4. output
wire SLEEP_CTRL_in_out = (cg_en)? (((current_state == CON_TO_FULLY && !(counter8 == 1 && counter9 == 28)) || current_state == LOAD_IMG_ONLY || current_state == LOAD_KER_IMG || current_state == LOAD_BOTH || current_state == IDLE) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_out;
GATED_OR GATED_in_out(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_out), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_out));

//5. distance
wire SLEEP_CTRL_in_dis = (cg_en)? (((current_state == CON_TO_FULLY && !(counter8 == 1 && counter9 == 28)) || current_state == LOAD_IMG_ONLY || current_state == LOAD_KER_IMG || current_state == LOAD_BOTH || current_state == IDLE) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_dis;
GATED_OR GATED_in_dis(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_dis), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_dis));

//6. con_to_fully
wire SLEEP_CTRL_in_cf = (cg_en)? (((current_state == LOAD_IMG_ONLY && in_valid) || current_state == OUTPUT || current_state == DISTANCE || current_state == LOAD_KER_IMG || current_state == LOAD_BOTH || current_state == IDLE) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_cf;
GATED_OR GATED_in_cf(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_cf), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_cf));
...
//counter
always@(posedge CLOCK_GATED_in_img or negedge rst_n) begin
    if(!rst_n) begin
        counter <= 0;
    end
    //LOAD
...
always@(posedge CLOCK_GATED_in_dis or negedge rst_n) begin
    if(!rst_n) begin
        counter12 <= 0;
    end
    //DISTANCE
...
always@(posedge CLOCK_GATED_in_cf or negedge rst_n) begin
    if(!rst_n) begin
        counter11 <= 0;
    end
    //CON_TO_FULLY
...
//Kernel
always@(posedge CLOCK_GATED_in_ker or negedge rst_n) begin
    if(!rst_n) begin
		for(i = 0 ; i < 3 ; i++) begin
            for(j = 0 ; j < 3 ; j++) begin
			    kernel_catch[i][j] <= 0; 
...
//Weight
always@(posedge CLOCK_GATED_in_wei or negedge rst_n) begin
    if(!rst_n) begin
		for(i = 0 ; i < 4 ; i++) begin
			weight_catch[i] <= 0; 
		end
...
//                   mult                       //
//==============================================//
// always@(posedge CLOCK_GATED_in_mult or negedge rst_n) begin
...
module comp (
    ...
    // always@(*) begin
    //     if((comp_in0 > comp_in1) && (comp_in0 > comp_in2) && (comp_in0 > comp_in3)) comp_out = comp_in0 ;
    //     else if((comp_in1 > comp_in0) && (comp_in1 > comp_in2) && (comp_in1 > comp_in3)) comp_out = comp_in1 ;
    //     else if((comp_in2 > comp_in1) && (comp_in2 > comp_in0) && (comp_in2 > comp_in3)) comp_out = comp_in2 ;
    //     else if((comp_in3 > comp_in1) && (comp_in3 > comp_in2) && (comp_in3 > comp_in0)) comp_out = comp_in3 ;
    // end

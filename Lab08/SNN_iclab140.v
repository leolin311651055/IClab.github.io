...
//3. 
wire SLEEP_CTRL_in_dis = (cg_en)? ((current_state != DISTANCE) ? 1'b1 : 1'b0) : 1'b0;
...
//4.
wire SLEEP_CTRL_in_cf = (cg_en)? ((current_state != CON_TO_FULLY) ? 1'b1 : 1'b0) : 1'b0;
...
//5.
wire SLEEP_CTRL_in_long = (cg_en)? ((current_state == IDLE || current_state == LOAD_BOTH || current_state == LOAD_KER_IMG || current_state == LOAD_IMG_ONLY) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_long;
GATED_OR GATED_in_long(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_long), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_long));

//6.
wire SLEEP_CTRL_in_long_2 = (cg_en)? ((current_state != CON_TO_FULLY) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_long_2;
GATED_OR GATED_in_long_2(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_long_2), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_long_2));

//7.
wire SLEEP_CTRL_in_long_3 = (cg_en)? ((current_state != CON_TO_FULLY) ? 1'b1 : 1'b0) : 1'b0;
wire CLOCK_GATED_in_long_3;
GATED_OR GATED_in_long_3(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_long_3), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_long_3));

// wire SLEEP_CTRL_in_img_2 = (cg_en)? ((current_state == CON_TO_FULLY || current_state == DISTANCE || current_state == OUTPUT) ? 1'b1 : 1'b0) : 1'b0;
// wire CLOCK_GATED_in_img_2;
// GATED_OR GATED_in_img_2(.CLOCK(clk), .SLEEP_CTRL(SLEEP_CTRL_in_img_2), .RST_N(rst_n), .CLOCK_GATED(CLOCK_GATED_in_img_2));
...
always@(posedge CLOCK_GATED_in_long or negedge rst_n) begin
    if(!rst_n) begin
        counter11 <= 0;
    end
    //CON_TO_FULLY
...
//                   div                       //
//==============================================//
always@(posedge CLOCK_GATED_in_long_3 or negedge rst_n) begin
    if(!rst_n) begin
        div_in0 <= 0;
    end
	//convolution
...
//                 Convolution                  //
//==============================================//
always@(posedge CLOCK_GATED_in_long_2 or negedge rst_n) begin
    if(!rst_n) begin
        for(i = 0 ; i < 2 ; i++) begin
			for(j = 0 ; j < 4 ; j++) begin
                for(k = 0 ; k < 4 ; k++) begin
